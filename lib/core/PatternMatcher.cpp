...
namespace cobra {
    ...
    namespace {
        ...
        TrySimplifyPatternSubtree(const Expr &expr, uint32_t bitwidth) {
            ...
            if (num_vars > 6) { return std::nullopt; }
        ...
        uint64_t PackBoolSig64(const std::vector< uint64_t > &sig) {
            uint64_t key = 0;
            for (size_t i = 0; i < sig.size(); ++i) {
                if (sig[i] != 0u) { key |= (uint64_t{ 1 } << i); }
            }
            return key;
        }
        ...
        // NPN4 lookup table for 4-variable Boolean functions.
        // 222 NPN equivalence classes cover all 65536 truth tables.
        // Generated by scripts/gen_npn4_table.py.
#include "Npn4Table.inc"
        ...
        std::optional< std::unique_ptr< Expr > > Match5varBoolean(uint32_t key) {
            ...
            if (f0 == f1) { return Match4varNpn(f0); }
            ...
            auto e0     = Match4varNpn(f0);
            auto e_diff = Match4varNpn(static_cast< uint16_t >(f0 ^ f1));
            if (!e0 || !e_diff) { return std::nullopt; }
            return Expr::BitwiseXor(std::move(*e0), Expr::BitwiseAnd(v(), std::move(*e_diff)));
        }

        // Shannon decomposition of 6-variable Boolean functions.
        // Splits on variable 5 (u): f(x,y,z,w,v,u) decomposes into cofactors
        // f0(x,y,z,w,v) = f(...,0) and f1(x,y,z,w,v) = f(...,1), each matched
        // by the 5-var decomposer. Covers all 2^64 non-constant functions.
        std::optional< std::unique_ptr< Expr > > Match6varBoolean(uint64_t key) {
            auto u = []() { return Expr::Variable(5); };

            const auto f0 = static_cast< uint32_t >(key & 0xFFFFFFFF);
            const auto f1 = static_cast< uint32_t >((key >> 32) & 0xFFFFFFFF);

            // Pure u or ~u
            if (f0 == 0x00000000 && f1 == 0xFFFFFFFF) { return u(); }
            if (f0 == 0xFFFFFFFF && f1 == 0x00000000) { return Expr::BitwiseNot(u()); }

            // One cofactor is constant 0
            if (f0 == 0x00000000) {
                auto e1 = Match5varBoolean(f1);
                if (!e1) { return std::nullopt; }
                return Expr::BitwiseAnd(u(), std::move(*e1));
            }
            if (f1 == 0x00000000) {
                auto e0 = Match5varBoolean(f0);
                if (!e0) { return std::nullopt; }
                return Expr::BitwiseAnd(Expr::BitwiseNot(u()), std::move(*e0));
            }
            ...
            if (f0 == 0xFFFFFFFF) {
                auto e1 = Match5varBoolean(f1);
                if (!e1) { return std::nullopt; }
                return Expr::BitwiseOr(Expr::BitwiseNot(u()), std::move(*e1));
            }
            if (f1 == 0xFFFFFFFF) {
                auto e0 = Match5varBoolean(f0);
                if (!e0) { return std::nullopt; }
                return Expr::BitwiseOr(u(), std::move(*e0));
            }

            // Cofactors equal: function doesn't depend on u
            if (f0 == f1) { return Match5varBoolean(f0); }

            // Cofactors complementary: f = f0 ^ u
            if (f0 == ~f1) {
                auto e0 = Match5varBoolean(f0);
                if (!e0) { return std::nullopt; }
                return Expr::BitwiseXor(std::move(*e0), u());
            }

            // General mux: f = f0 ^ (u & (f0 ^ f1))
            auto e0     = Match5varBoolean(f0);
            auto e_diff = Match5varBoolean(f0 ^ f1);
            if (!e0 || !e_diff) { return std::nullopt; }
            return Expr::BitwiseXor(std::move(*e0), Expr::BitwiseAnd(u(), std::move(*e_diff)));
        }
    ...
    MatchPattern(const std::vector< uint64_t > &sig, uint32_t num_vars, uint32_t bitwidth) {
        ...
        if (num_vars == 1) {
            ...
            // 4-var Boolean: NPN equivalence class table lookup
            result = Match4varNpn(static_cast< uint16_t >(PackBoolSig(sig)));
        } else if (num_vars == 5 && IsBooleanSig(sig)) {
            // 5-var Boolean: Shannon decomposition into two 4-var NPN lookups
            result = Match5varBoolean(PackBoolSig(sig));
        } else if (num_vars == 6 && IsBooleanSig(sig)) {
            // 6-var Boolean: Shannon decomposition into two 5-var lookups
            result = Match6varBoolean(PackBoolSig64(sig));
